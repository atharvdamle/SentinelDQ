...
class TypeChecker:
    ...
    def validate(self, event: Dict[str, Any]) -> List[ValidationFailure]:
        ...
        for rule in self.rules:
            ...
            error_message = rule.get("error_message", f"Field '{field}' has incorrect type")
...
def check_field_type(
    event: Dict[str, Any], field_path: str, expected_type: str, severity: Severity = Severity.CRITICAL
    ...
    """
    checker = TypeChecker([{"field": field_path, "expected_type": expected_type, "severity": severity.value}])
...
def validate_types(
    event: Dict[str, Any], type_rules: Dict[str, str], severity: Severity = Severity.CRITICAL
    ...
    """
    rules = [
        {"field": field, "expected_type": type_name, "severity": severity.value}
